import { useState } from "react";

const SignUp = () => {
  const [firstName, setFirstName] = useState("");
  const [lastName, setLastName] = useState("");
  const [email, setEmail] = useState("");
  const [contactNo, setContactNo] = useState("");
  const [briefInfo, setBriefInfo] = useState("");
  const [termsAndConditions, setTermsAndConditions] = useState(false);

  const initialCheckboxState = {
    MarketingBrandingProfessional: false,
    GraphicArtist: false,
    ProjectManagementMentor: false,
    InstructionalDesignFunctionalLead: false,
    HRGeneralist: false,
    PMOOfficeCoordinator: false,
    FundraisingGrantAcquisitionAssociate: false,
    EditorWriter: false,
    DatabaseFunctionalLead: false,
    GrantAcquisitionResearcherWriter: false,
    BoardMemberFinance: false,
    DirectorFoundationCommunication: false,
    ExecutiveSecretary: false,
  };

  const [checkboxes, setCheckboxes] = useState(initialCheckboxState);

  const handleCheckboxChange = (event) => {
    const { name, checked } = event.target;
    setCheckboxes((prevCheckboxes) => ({
      ...prevCheckboxes,
      [name]: checked,
    }));
  };

  const filterCheckboxes = (data) => {
    const trueCheckboxes = Object.keys(data.checkboxes)
      .filter((key) => data.checkboxes[key])
      .join(", ");

    return { ...data, checkboxes: trueCheckboxes };
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    const userInput = {
      firstName,
      lastName,
      email,
      contactNo,
      briefInfo,
      checkboxes,
    };

    const filteredData = filterCheckboxes(userInput);

    const data = new FormData();
    data.append("firstName", filteredData.firstName);
    data.append("lastName", filteredData.lastName);
    data.append("email", filteredData.email);
    data.append("contactNo", filteredData.contactNo);
    data.append("briefInfo", filteredData.briefInfo);
    data.append("checkboxes", filteredData.checkboxes);

    const response = await fetch(
      "https://script.google.com/macros/s/AKfycbwwlgVzJv42ERjRKso-pk8Xu9HD3GZl4nxdEltQ3nEzNcX_f9U0_U2ww4kZPlqZi51_Yg/exec",
      {
        method: "POST",
        body: data,
        muteHttpExceptions: true,
      }
    );

    if (response.ok) {
      setFirstName("");
      setLastName("");
      setEmail("");
      setContactNo("");
      setBriefInfo("");
      setCheckboxes(initialCheckboxState);
      setTermsAndConditions(false);
    } else {
      alert("There was an error sending your message.");
    }
  };

  return (
    <div className="w-screen h-full md:min-h-[650px] md:mt-[5rem] lg:px-5 bg-white flex justify-center">
      <div className="w-full max-w-[3000px] h-full flex flex-col items-start md:items-center justify-center font-bold">
        <div className="max-w-[1600px] mx-[1rem] md:mx-[2rem] lg:mx-[8rem]">
          <h2 className="text-[2rem] md:text-[3.5rem] lg:text-5xl md:text-center my-[2rem]">
            Volunteer Sign-Up & Needs
          </h2>
          <form className="space-y-4 mt-2" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-5">
              <div>
                <input
                  type="text"
                  id="firstName"
                  placeholder="First Name"
                  className="block w-full px-3 py-3 font-normal bg-gray-200 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm placeholder:italic placeholder:text-gray-500"
                  value={firstName}
                  onChange={(e) => setFirstName(e.target.value)}
                  required
                />
              </div>
              <div>
                <input
                  type="text"
                  id="lastName"
                  placeholder="Last Name"
                  className="block w-full px-3 py-3 font-normal bg-gray-200 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm placeholder:italic placeholder:text-gray-500"
                  value={lastName}
                  onChange={(e) => setLastName(e.target.value)}
                  required
                />
              </div>
              <div>
                <input
                  type="email"
                  id="email"
                  placeholder="Email"
                  className="block w-full px-3 py-3 font-normal bg-gray-200 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm placeholder:italic placeholder:text-gray-500"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  required
                />
              </div>
              <div>
                <input
                  type="tel"
                  id="contactNo"
                  placeholder="Phone Number"
                  className="block w-full px-3 py-3 font-normal bg-gray-200 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm placeholder:italic placeholder:text-gray-500"
                  value={contactNo}
                  onChange={(e) => setContactNo(e.target.value)}
                />
              </div>
            </div>
            <div>
              <textarea
                id="briefInfo"
                placeholder="Brief Bio"
                className="block w-full px-3 py-3 font-normal bg-gray-200 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm placeholder:italic placeholder:text-gray-500"
                rows="4"
                value={briefInfo}
                onChange={(e) => setBriefInfo(e.target.value)}
                required
              ></textarea>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="MarketingBrandingProfessional"
                    checked={checkboxes.MarketingBrandingProfessional}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Marketing/Branding Professional
                  </span>
                </label>
              </div>

              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="GraphicArtist"
                    checked={checkboxes.GraphicArtist}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Graphic Artist
                  </span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 min-w-7 min-h-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="ProjectManagementMentor"
                    checked={checkboxes.ProjectManagementMentor}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Project Management Mentor/Coach (only those with significant
                    PM experience)
                  </span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="InstructionalDesignFunctionalLead"
                    checked={checkboxes.InstructionalDesignFunctionalLead}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Instructional Design Functional Lead
                  </span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="HRGeneralist"
                    checked={checkboxes.HRGeneralist}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">HR Generalist</span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="PMOOfficeCoordinator"
                    checked={checkboxes.PMOOfficeCoordinator}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    PMO Office Coordinator
                  </span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="FundraisingGrantAcquisitionAssociate"
                    checked={checkboxes.FundraisingGrantAcquisitionAssociate}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Fundraising / Grant Acquisition Associate
                  </span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="EditorWriter"
                    checked={checkboxes.EditorWriter}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">Editor/Writer</span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 min-w-7 min-h-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="DatabaseFunctionalLead"
                    checked={checkboxes.DatabaseFunctionalLead}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Database Functional Lead (guides and mentors database
                    designers/developers)
                  </span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="GrantAcquisitionResearcherWriter"
                    checked={checkboxes.GrantAcquisitionResearcherWriter}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Grant Acquisition Researcher/Writer
                  </span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="BoardMemberFinance"
                    checked={checkboxes.BoardMemberFinance}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Board Member: Finance (experienced CPA)
                  </span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="DirectorFoundationCommunication"
                    checked={checkboxes.DirectorFoundationCommunication}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Director: Foundation Communication
                  </span>
                </label>
              </div>
              <div>
                <label className="inline-flex items-center">
                  <input
                    type="checkbox"
                    className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                    name="ExecutiveSecretary"
                    checked={checkboxes.ExecutiveSecretary}
                    onChange={handleCheckboxChange}
                  />
                  <span className="ml-2 text-sm font-light">
                    Executive Secretary
                  </span>
                </label>
              </div>
            </div>
            <div>
              <label className="inline-flex items-center mt-10">
                <input
                  type="checkbox"
                  className="peer relative appearance-none h-7 w-7 rounded-sm bg-gray-200 checked:bg-gray-500 after:absolute after:left-0 after:top-0 after:h-full after:w-full after:bg-[url('data:image/svg+xml;base64,')] after:bg-[length:70px] after:bg-center after:bg-no-repeat"
                  name="termsAndConditions"
                  checked={termsAndConditions}
                  onChange={(e) => setTermsAndConditions(e.target.checked)}
                  required
                />
                <span className="ml-2 text-md font-light">
                  Agree with terms and conditions
                </span>
              </label>
            </div>
            <div className="flex justify-center lg:justify-start">
              <button className="block mt-7 mb-[4rem] w-[297px] h-[76px] bg-primary500 rounded-full hover:bg-primary300 transition duration-[300ms] ease-linear text-[1rem] text-white font-semibold">
                Submit
              </button>
            </div>
          </form>
        </div>
      </div>
    </div>
  );
};

export default SignUp;
