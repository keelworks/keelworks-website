import doner from '../../../assets/images/become-doner.png'

const BecomeDonor = () => {
  return (
    <div className="w-screen min-h-screen mt-[5rem] flex justify-center">
  <div className="w-full max-w-[3000px] min-h-full text-center flex flex-col items-center bg-[#F6F7F9]">
    <div className="h-1/5 flex items-center justify-center">
      <h1 className="font-bold text-[#2E2E2E] text-[50px]">Become a Donor</h1>
    </div>
    <div className="w-full max-w-[1300px] flex flex-col md:flex-row h-4/5 mb-10">
      <div className='flex justify-center items-center w-full md:w-1/2 h-full'>
        <img src={doner} alt="Donor" className='w-full h-full object-cover' />
      </div>
      <div className='flex justify-center items-center w-full md:w-1/2 '>
        <div className='p-8 text-start bg-primary500 w-full text-[#2E2E2E] flex flex-col justify-center '>
          <p className='font-bold text-[25px] md:text-[50px] leading-tight'>
            The quickest and fastest way to help is by becoming a donor.
          </p>
          <p className='italic text-[20px] md:text-[30px] mt-2'>
            Whether it is one time, or on a regular basis, every bit helps!
          </p>
        </div>
      </div>
    </div>
  </div>
</div>




  );
};
export default BecomeDonor;
